...
> [!TIP]
> `mask | (1 << i)` adds city `i` to the visited set, `mask & (1 << i)` checks if it's already there. Both are direct applications of the bit tricks table in [Number Theory](../basic-foundation/number-theory.md), this is exactly why that file exists as a prerequisite instead of being folded into DP directly.
