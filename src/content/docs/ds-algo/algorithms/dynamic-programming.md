...
- **Overlapping subproblems**: the same smaller subproblem gets solved multiple times if you just did plain recursion. This is exactly the "waste" identified in the recursion tree from [Recursion](../basic-foundations/recursion.md), `fib(2)` getting computed twice from two different branches.
