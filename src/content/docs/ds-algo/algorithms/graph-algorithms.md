...
Both visit every reachable node exactly once, O(V + E), covered in the graph-specific row of the [Complexity Analysis](../basic-foundations/complexity-analysis.md) table. They differ in *order*, and that order is the entire reason to pick one over the other.
